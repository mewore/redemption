...
if [ "${is_test_web}" == 0 ]; then
  ...
  if wait "${windows_pid}"; then
    make_zip "${version_dir}/${WINDOWS_BASE_DIR}" "${version_dir}/${export_name} - x64.zip" &
  else
    echo "Windows export failed: file://${windows_log}"
  fi

  if wait "${linux_pid}"; then
    make_tarball "${version_dir}/${LINUX_BASE_DIR}" "${version_dir}/${lowercase_export_name}-x64.tar.gz" \
      "${LINUX_SUB_DIR_NAME}" &
  else
